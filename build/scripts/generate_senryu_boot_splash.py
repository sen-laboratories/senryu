...
def generate_split_neon(input_path, out_dir):
    idx_match = re.search(r'_(\d+)\.png$', input_path)
    index = int(idx_match.group(1)) if idx_match else 0

    base_img = Image.open(input_path).convert("RGB").resize((SPLASH_WIDTH, SPLASH_HEIGHT), Image.LANCZOS)
    p_img = Image.new('P', (1,1)); p_img.putpalette(pal)

    if index == 0:
        base_img.quantize(palette=p_img).save(os.path.join(out_dir, "bg_static.png"))
        return

    # Process both parts
    for part_name, bbox in [("sen", SEN_BBOX), ("ryu", RYU_BBOX)]:
        crop = base_img.crop(bbox)
        for state, level in [("cold", 0.3), ("warm", 1.0), ("vibrant", 1.5)]:
            enhancer = ImageEnhance.Brightness(crop)
            final = enhancer.enhance(level).quantize(palette=p_img)
            final.save(os.path.join(out_dir, f"{part_name}_{index}_{state}.png"))
...
def generate_neon_states(input_path, out_dir):
    ...
    # Load and scale
    base_img = Image.open(input_path).convert("RGB").resize((1024, 768), Image.LANCZOS)

    # Remove AI Watermark
    w, h = base_img.size
    for x in range(int(w*0.82), w):
        for y in range(int(h*0.92), h):
            base_img.putpixel((x, y), (0,0,0))

    p_img = Image.new('P', (1,1)); p_img.putpalette(pal)

    # If it's the background, just save one version
    if index == 0:
        final = base_img.quantize(palette=p_img, dither=Image.FLOYDSTEINBERG)
        final.save(os.path.join(out_dir, "stage_0.png"))
        return

    # For stages, create 3 luminosity levels
    crop = base_img.crop(STAGE_BBOX)

    # 1. Warm (Standard)
    warm = crop.quantize(palette=p_img, dither=Image.FLOYDSTEINBERG)
    warm.save(os.path.join(out_dir, f"stage_{index}_warm.png"))

    # 2. Cold (Dimmed)
    cold_enhancer = ImageEnhance.Brightness(crop)
    cold = cold_enhancer.enhance(0.4).quantize(palette=p_img, dither=Image.FLOYDSTEINBERG)
    ...
    vibrant = vib_enhancer.enhance(1.4).quantize(palette=p_img, dither=Image.FLOYDSTEINBERG)
...
if __name__ == "__main__":
    ...
    generate_split_neon(sys.argv[1], sys.argv[2])
